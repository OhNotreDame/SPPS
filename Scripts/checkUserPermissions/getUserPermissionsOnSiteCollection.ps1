...
.SYNOPSIS
	Get all the permissions for all the securable objects (SPWeb, SPList, SPFolder or SPListItem) for a specific User on a specific SPSite (Site Collection), and generate CSV files with the results.
...
.EXAMPLE
	getUserPermissionsOnSiteCollection.ps1 -siteUrl <siteURL> -userName <userName> -notClaims
...
try 
{
	...
	if($site -ne $null) 
	{
		...
		if ($notClaims)
		{
			...
			$permList += getUserPermissionsOnSPSite -siteCollURL $siteURL -userName $userName
		...
		#Step 2: Site Collection and Subsites
		$permList += getUserPermissionsOnSPSite -siteCollURL $siteURL -userName $userNameClaims
